...
The historical successful responses, URLs, query filters and all five acquisition outcomes are frozen in `docs/paper/free-data-2026-09-06/support_history_2026-09-06.json`. The shipped `backend/data/support_price_history.json` was subsequently extended; see `free-comtrade-evidence-2026-09-07.md`. SHA-256: `4d0dccdf351ea3ecbebca9233da5cd6a97d5a790ecab313749fcd9b6c8af5896`. The retained `isAggregate=true` refers to the requested world/all-transport aggregate; preview returns null HS-level descriptors, so exact six-digit `cmdCode` is checked and a contradictory level is rejected. Neither estimated weights nor duplicate rows are accepted. Boolean/non-finite quantities, wrong filters, mismatched counts, missing response evidence, foreign endpoints and forged HTTP statuses are rejected. Source labels are rebuilt from the checked catalog rather than trusted from a supplied file.
